...
public class JdbcUserDao implements UserDao {
    ...
    public User saveUser(String userName, String firstName, String lastName, 
    		String email, String password, String securityQuestion, String securityAnswer, String role, String category1, String category2, String neighborhood1, String neighborhood2) {
        byte[] salt = passwordHasher.generateRandomSalt();
        String hashedPassword = passwordHasher.computeHash(password, salt);
        String saltString = new String(Base64.encode(salt));
        long newId = jdbcTemplate.queryForObject(
                "INSERT INTO users(username, firstname, lastname, email, password, salt, role, security_question, security_answer, category1, category2, neighborhood1, neighborhood2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id", Long.class,
                userName, firstName, lastName, email, hashedPassword, saltString, role, securityQuestion, securityAnswer, category1, category2, neighborhood1, neighborhood2);
    ...
    public List<User> getAllUsers() {
        ...
        String sqlSelectAllUsers = "SELECT id, username, firstname, lastname,  email, role, security_question, security_answer, category1, category2, neighborhood1, neighborhood2 FROM users";
    ...
    private User mapResultToUser(SqlRowSet results) {
        ...
        user.setSecurityQuestion(results.getString("security_question"));
        user.setSecurityAnswer(results.getString("security_answer"));
    ...
    @Override
    public User getValidUserWithEmail(String email) {
        String sqlSelectUserByEmail = "SELECT id, firstname, lastname, email, username, role, security_question, security_answer, category1, category2, neighborhood1, neighborhood2 FROM users WHERE email = ?";
    ...
    public User getUserByUsername(String username) {
        String sqlSelectUserByUsername = "SELECT id, firstname, lastname, email, username, role, security_question, security_answer, category1, category2, neighborhood1, neighborhood2 FROM users WHERE username = ?";
